...
#### c) *fit a linear model allowing for interaction terms using LASSO (again using glmnet). Again calculate the MSE of this model and compare it to the one from a)*  
...
Y <- Prostate$lpsa
X.int <- model.matrix(lpsa ~ .^2, data = Prostate) 


lasso.int<- cv.glmnet(X.int,Y) #cross validation to estimate lambda 

lambda <- lasso.int$lambda.1se
lambda

fit.lasso.int <- glmnet(X.int, Y, lambda=lambda)
...
We see that for most interaction terms the coefficient is set to 0 by LASSO. 
Next we estimated the MSE "by hand" as in a). 

```{r}

leave_one_out_CV <- function(lambda){ #same function as in a) just X is changed to X.int 
  n <- nrow(X.int)
  p <- ncol(X.int) # Just FYI
  ...
  for (i in 1:n){
    ...
    # Then we predict the value with the fitted model for the missing observation i
    prediction <- predict(prostate.lasso, newx = X.int[i, , drop=FALSE], s=lambda, type="link")
    #type: Type of prediction required. Type "link" gives the linear predictors for "binomial" or "multinomial" models
    #drop=FALSE: trick to avoid automatic conversion in vector or other datastructure of 1D data when we need a dataframe or a matrix
...
MSE <-leave_one_out_CV(lambda)
print(MSE)


```

The MSE for the model including interaction terms is slightly higher compared to the model seen in a).  
